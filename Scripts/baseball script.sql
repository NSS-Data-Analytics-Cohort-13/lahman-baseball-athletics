...
--answer: Chris Owings (Christopher Scott) at 91%
--50 rows; SUM sb and cs to account for all players from all teams
...
FROM pct
--answer:27%
--25.53% with 12/47 (1981 and 1994 strikes)
...
SELECT tsnwinners.playerid, tsnwinners.lgid, tsnwinners.yearid, m.teamid, t.name, p.namefirst, p.namelast
...
ON tsnwinners.playerid=m.playerid AND tsnwinners.yearid=m.yearid
INNER JOIN teams AS t --to get team name
ON m.teamid=t.teamid AND t.yearid=m.yearid
INNER JOIN people AS p --to get full name
ON tsnwinners.playerid=p.playerid
